      ...
      if (result.err != null) {
        ...
        bcrypt.compare(password, pass, function (err, result) {
          if (result == true) {
            ...
            generateAccessToken(userid, email, userType).then((token) => {
              return res.status(202).send({
                token: token,
                username: username,
                id: userid,
                userType: userType,
                profile_pic_link: pfp,
              }); //if the js sees the 202 status, keep the name in session storagee
            });
          } else {
            // console.log("bruh");
...
app.get("/topNav", checkToken, (req, res) => {
...
app.post("/user/biometrics/create", (req, res) => {
  //set user biometrics
  // console.log("ok");
...
// ! NOT IN USE YET
// TODO: REFACTOR CODE TO USE REQ.TOKEN
app.get("/user/biometrics", (req, res) => {
  //set user biometrics
  const id = req.body.userid;
  ...
  verifyJWT(req.token).then((decoded) => {
    ...
    if (userType === 2) {
      ...
      db.assignLecturerToStudent(studentID, lecturerID, (result) => {
        ...
        return res.status(202).send({ result: "Added student " + studentID });
      });
    } else {
      // console.log("access denied");
      res.sendStatus(403);
  ...
  verifyJWT(req.token).then((decoded) => {
    ...
    if (id != null) {
      ...
      db.updateProfile(id, username, pfp, email, (result) => {
        ...
        generateAccessToken(id, email, userType).then((token) => {
          return res
            .status(202)
            .send({ token: token, username: username, id: id }); //if the js sees the 202 status, keep the name in session storagee
        });
      });
    } else {
      // console.log("access denied");
...
// TODO: REFACTOR CODE TO USE REQ.TOKEN
app.get("/article/sidebar", (req, res) => {
  //get articles summaries for sidebar
...
app.post("/quiz/submit", checkToken, (req, res) => {
  //submit quiz results
